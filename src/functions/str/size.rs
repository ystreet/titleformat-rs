...
pub fn longer(args: Vec<Value>) -> Result<Value, Error> {
    match args.len() {
        2 => Ok(value_string("", args[0].val.len() > args[1].val.len())),
...
mod tests {
    ...
    fn test_len() {
        assert_eq!(
            len(vec![value_string("12345", true)]).unwrap(),
            value_string("5", true)
        );
    }
    ...
    fn test_longer() {
        ...
        assert_eq!(
            longer(vec![value_string("12345", true), value_string("123", true)]).unwrap(),
            value_string("", true)
        );
        assert_eq!(
            longer(vec![value_string("123", true), value_string("12345", true)]).unwrap(),
            value_string("", false)
        );
    }
}
