...
pub fn stripprefix(args: Vec<Value>) -> Result<Value, Error> {
    ...
    match split_first(
        s.clone(),
        (args[1..]).iter().map(|val| val.val.clone()).collect(),
    ) {
        None => {}
        Some((_, new)) => s = new,
    }
...
pub fn cut(args: Vec<Value>) -> Result<Value, Error> {
    ...
    let len = if str_len < 0 { 0 } else { str_len as usize };
...
mod tests {
    ...
    fn wrong_n_arguments() {
        assert_eq!(
            stripprefix(vec![]).err().unwrap(),
            InvalidNativeFunctionArgs(String::from("stripprefix"), 0)
        );
        assert_eq!(
            swapprefix(vec![]).err().unwrap(),
    ...
    fn test_stripprefix() {
        assert_eq!(
            stripprefix(vec![value_string("A thing", true)]).unwrap(),
    ...
    fn test_swapprefix() {
        assert_eq!(
            swapprefix(vec![value_string("A thing", true)]).unwrap(),
    ...
    fn test_cut() {
        assert_eq!(
            cut(vec![value_string("A thing", true), value_string("2", true)]).unwrap(),
            value_string("A ", true)
        );
        assert_eq!(
            cut(vec![
                value_string("A thing", true),
                value_string("non", true)
            ])
            .unwrap(),
            value_string("", true)
        );
